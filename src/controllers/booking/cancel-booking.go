...
func CancelBooking(c *gin.Context) {
  ...
  if err := db.Model(&models.Booking{}).Where("id = ?", input.BookingID).First(&booking).Error; err != nil {
    utils.JSONResponse(c, 400, false, "Invalid booking", nil)
    return
  }
  ...
  if booking.Status != "pending" {
    utils.JSONResponse(c, 400, false, "Invalid booking status", nil)
    return
  }
  ...
  utils.JSONResponse(c, 200, true, "Booking canceled", nil)
}
